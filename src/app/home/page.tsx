  ...
  return (
    <>
      <div className="flex-1">
        <div className="flex items-center gap-3">
          <div className="relative h-10 w-10 overflow-hidden rounded-lg bg-gray-100 dark:bg-gray-800">
            <Image src={`https://avatars.githubusercontent.com/${repo.fullName.split("/")[0]}`} alt={repo.name} fill className="object-cover" />
          </div>
          <div>
            <h3 className="font-medium text-gray-900 dark:text-gray-200">{repo.fullName}</h3>
            <p className="text-sm text-gray-500 dark:text-gray-400">{repo.stars.toLocaleString()} stars</p>
          </div>
        </div>
        <p className="mt-4 line-clamp-2 text-sm text-gray-600 dark:text-gray-400">{repo.description}</p>
      </div>
      ...
      <div className="mt-6 flex items-center justify-between">
        ...
        <VoteButton repoId={repo.id} currentVotes={repo.voteCount} isCurrentVote={repo.id === currentVoteId} repoName={repo.fullName} />
      </div>
    </>
  )
}
    ...
    <main className="min-h-screen bg-gray-50 dark:bg-gray-950">
      ...
      <nav className="fixed top-0 z-50 w-full border-b border-gray-200/40 bg-white/80 backdrop-blur-md dark:border-gray-800/40 dark:bg-gray-950/70">
        <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
          <Link href="/home" className="transition-opacity hover:opacity-80">
            <Image src="/images/logos/company-logo-dark.svg" alt="Company Name" width={140} height={38} className="block h-10 w-auto dark:hidden" priority />
            <Image src="/images/logos/company-logo.svg" alt="Company Name" width={140} height={38} className="hidden h-10 w-auto dark:block" priority />
          </Link>
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              {session.user?.image && (
                <Image src={session.user.image} alt={session.user.name ?? "Profile"} width={32} height={32} className="rounded-full ring-2 ring-gray-200 dark:ring-gray-800" />
              )}
              <span className="hidden text-sm font-medium text-gray-900 dark:text-gray-200 sm:inline">{session.user?.name}</span>
            </div>
            <div className="h-6 w-px bg-gray-200 dark:bg-gray-800" />
            <SignOutButton />
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="mb-12">
          ...
          <p className="mt-2 text-gray-600 dark:text-gray-400">Vote for your favorite open-source projects</p>
        </div>

        <Suspense fallback={<LoadingGrid />}>
          <RepositoryGrid leaderboard={leaderboard} currentVoteId={currentVoteId ?? undefined} />
        </Suspense>
      </div>
    </main>
  )
